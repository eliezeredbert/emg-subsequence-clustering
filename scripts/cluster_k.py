...
def cluster_analysis(window_size, dilation, step_size):
    ...
    # Load feature data
    feature_file = FEATURES_DIR / f"emg_len{window_size}_dil{dilation}_step{step_size}.parquet"
    ...
    cluster_output = CLUSTERS_RESULTS_DIR / f"emg_len{window_size}_dil{dilation}_step{step_size}_clusters.parquet"
    conn = duckdb.connect()
...
def plot_results(results_df, window_size, dilation, step_size):
    ...
    plt.suptitle(f'Clustering Analysis: window_size={window_size}, dilation={dilation}, step_size={step_size}')
    plt.tight_layout()
    
    CLUSTERS_RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    output_file = CLUSTERS_RESULTS_DIR / f"cluster_analysis_len{window_size}_dil{dilation}_step{step_size}.png"
...
if __name__ == "__main__":
    ...
    for step_size in step_sizes:
        ...
        print(f"Best parameters: window_size={window_size}, dilation={dilation}, score={max_score:.6f}")
        ...
        # Save CSV results
        CLUSTERS_RESULTS_DIR.mkdir(parents=True, exist_ok=True)
        output_csv = CLUSTERS_RESULTS_DIR / f"cluster_results_len{window_size}_dil{dilation}_step{step_size}.csv"
        ...
        all_results.append({
            ...
            'dilation': dilation,
            'max_silhouette_score': max_score,
            'k2_silhouette': results_df[results_df['k'] == 2]['silhouette'].iloc[0],
            'k5_silhouette': results_df[results_df['k'] == 5]['silhouette'].iloc[0],
    ...
    # Summary
    print("\nSUMMARY OF DETAILED CLUSTERING RESULTS:")
    print("=" * 80)
    print("Step | Window | Dilation | Max Score | k=2 Score | k=5 Score")
    print("-" * 80)
    
    for result in all_results:
        print(f"{result['step_size']:4d} | {result['window_size']:6d} | "
              f"{result['dilation']:8d} | {result['max_silhouette_score']:9.6f} | "
              f"{result['k2_silhouette']:9.6f} | {result['k5_silhouette']:9.6f}")
    
    print(f"\nProcessed {len(all_results)} step sizes successfully!")