...
def process_dictionary_file(file_path, output_dir=None):
    ...
    for _, row in df.iterrows():
        ...
        results.append({
            'label': label,
            'phonics': alphabet_string
        })
        
        print(f"  Label {label}: {sequence_values[:10]}... -> {grouped_sequence} -> {alphabet_string}")
...
def process_all_dictionary_files(input_folder=None, output_folder=None):
    """Process all dictionary CSV files in the specified or default directory"""
    
    print("PHONICS PROCESSING - Converting Cluster Sequences to Alphabet Patterns")
    print("=" * 80)
    
    # Use custom input folder or default
    if input_folder:
        dictionary_dir = Path(input_folder)
        print(f"Using custom input folder: {dictionary_dir.absolute()}")
    else:
    ...
    if output_folder:
        print(f"Output folder: {Path(output_folder).absolute()}")
    else:
    ...
    for file_path in sorted(dictionary_files):
        try:
            output_file = process_dictionary_file(file_path, output_folder)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert cluster dictionary files to phonics patterns')
    parser.add_argument('-i', '--input-folder', type=str, 
                       help='Input folder containing step*_dictionary.csv files (default: ../result/dict)')
    parser.add_argument('-o', '--output-folder', type=str,
                       help='Output folder for phonics files (default: same as input folder)')
    
    args = parser.parse_args()
    
    process_all_dictionary_files(args.input_folder, args.output_folder)
